...
__global__ void kConvolveTest(float* image, float* filter, float* target,
        int n, int c, int n_ftr, int im_h, int im_w, int ftr_h, int ftr_w) {
    ...
    const int ftr_im_size = ftr_c_size * n_ftr;

    const int t_w = tid % target_w;
    const int t_h = (tid / target_w) % target_h;
    const int t_c = (tid / target_c_size) % n_ftr;
    ...
    for (int k = 0; k < c; k++)
        for (int i = 0; i < im_h; i++)
            for (int j = 0; j < im_w; j++) {
                s += image[t_n * image_im_size + k * image_c_size + (t_h + i) * im_w + t_w] * \
